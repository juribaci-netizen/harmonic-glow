...
export function VideosView({initialVideos}:{initialVideos:Video[]}){
 ...
 if(selected)return <div className="space-y-4 pb-4">
  ...
  <Card className="overflow-hidden rounded-2xl border-0 bg-black shadow-lg"><div className="relative aspect-video">{selected.thumbnailUrl?<img src={selected.thumbnailUrl} alt="" className="h-full w-full object-cover opacity-80"/>:<div className="h-full w-full bg-gradient-to-br from-[#17233d] to-[#66758f]"/>}<a href={selected.url||"#"} target="_blank" rel="noreferrer" className="absolute inset-0 flex items-center justify-center"><span className="flex h-16 w-16 items-center justify-center rounded-full bg-white text-[#17233d] shadow-xl"><Play className="ml-1 h-6 w-6 fill-current"/></span></a></div></Card>
  <div className="rounded-2xl bg-[#f5f7fa] p-4"><p className="text-sm font-medium">Prehrávanie</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Video prehráva Slovenská filharmónia na svojom oficiálnom archíve. Ťukni na prehrávanie a otvorí sa konkrétny koncert.</p>{selected.conductor&&<p className="mt-3 text-xs"><span className="text-muted-foreground">Dirigent:</span> {selected.conductor}</p>}<a href={selected.url||"#"} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-[#17233d]">Otvoriť na SF <ExternalLink className="h-3 w-3"/></a></div>
 </div>
