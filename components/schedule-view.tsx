...
export function ScheduleView({activities,seedSeason}:{activities:Activity[];seedSeason:()=>Promise<{seeded:boolean;count?:number}>}){
 ...
 return <div className="flex flex-col gap-5 pb-3"><header><h1 className="font-serif text-[28px] font-semibold">{t.seasonSchedule}</h1><p className="mt-1 text-xs text-muted-foreground">24. 8. 2026 – 3. 1. 2027 · Slovenská filharmónia</p></header>
  ...
  <section className="space-y-3">{visible.length===0?<Card className="p-8 text-center text-sm text-muted-foreground">{t.noUpcoming}</Card>:visible.map(a=>{const s=typeStyles[a.type]??typeStyles.other;const cancelled=/zruš/i.test(`${a.title} ${a.notes??""}`);return <Card key={a.id} className={`rounded-xl p-4 shadow-none ${cancelled?"border-red-300 bg-red-50/90":""}`}><div className="flex gap-3"><span className={`mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full ${cancelled?"bg-red-500":s.dot}`}/><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><p className={`text-sm font-semibold ${cancelled?"text-red-800 line-through decoration-red-500/70":""}`}>{fmt(a)||a.title}</p><p className={`mt-0.5 text-xs ${cancelled?"font-semibold text-red-700":"text-foreground"}`}>{a.title}</p></div><span className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${cancelled?"border-red-300 bg-red-100 text-red-700":s.badge}`}>{cancelled?"ZRUŠENÉ":label(a.type)}</span></div>{cancelled?<p className="mt-2 text-[12px] font-bold uppercase tracking-[0.08em] text-red-700">Zrušená</p>:<>{a.conductor&&<p className="mt-2 text-[11px] font-medium">Diriguje: {a.conductor}</p>}{a.venue&&<p className="mt-1 flex items-center gap-1 text-[11px] text-muted-foreground"><MapPin className="h-3 w-3"/>{a.venue}</p>}{a.program&&<p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">{a.program}</p>}{a.notes&&<p className="mt-2 rounded-lg bg-[#f7f9fc] p-2 text-[11px] leading-relaxed text-muted-foreground">{a.notes}</p>}</>}</div></div></Card>})}</section>
 </div>
}
